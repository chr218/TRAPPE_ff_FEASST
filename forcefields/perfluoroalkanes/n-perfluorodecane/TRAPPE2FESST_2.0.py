    ...
    for i,C in enumerate(atoms_C):
        ...
        if C.symbol == 'C':
            for j,H in enumerate(atoms_H):
                if np.linalg.norm(C.position-H.position) < 1.5:
                    hybridization += 1
...
TRAPPE_fname = 'trappe_parameters_75.txt'
TRAPPE_atoms, TRAPPE_bonds, TRAPPE_name = get_TRAPPE_params(TRAPPE_fname)

#ASE
#Convert Smiles string into Atoms object.
SMILES = 'FC(F)(C(F)(F)C(F)(F)C(F)(F)C(F)(F)C(F)(F)C(F)(F)C(F)(F)C(F)(F)F)C(F)(F)F'
#atoms = pubchem_atoms_search(smiles=SMILES)
atoms = read_proteindatabank('n-perfluorodecane.pdb')
