...
def get_psuedoatoms(atoms_obj,TRAP_atms,TRAP_bonds):
    ...
    del atoms_H[[atom.index for atom in atoms_H if not atom.symbol=='F']]
    
    atoms_C = atoms_obj.copy()
    del atoms_C[[atom.index for atom in atoms_C if atom.symbol=='F' and atom.index not in H_index]]   

    for i,C in enumerate(atoms_C):
        hybridization = 0
        if C.symbol == 'C':
            for j,H in enumerate(atoms_H):
                if np.linalg.norm(C.position-H.position) < 1.4:
                    hybridization += 1
...
def Order_atoms_wrt_TRAPPE(ASE_atms, ASE_geom, TRAP_bonds,TRAP_atms):
    ...
    for i, atm in enumerate(ASE_atms):
        if atm.symbol =='C':              
            if atm.tag == 0:
                ...
                ASE_hybrid_list.append('CF')
            else:
                ASE_hybrid_list.append('CF' + str(atm.tag))
        elif atm.symbol != 'C':
    ...
    for i, dist in enumerate(TRAP_bond_dist_list):
        ...
        print(TRAP_bond_hybrids[i],ASE_bond_hybrids[indx])      
        ...
        for j,TRAPE_hybrid in enumerate(TRAP_bond_hybrids[i]):
            for k, ASE_hybrid in enumerate(ASE_bond_hybrids[indx]):
                ...
                if TRAPE_hybrid not in ASE_bond_hybrids[indx]:
                    print(dist)
                    print(TRAPE_hybrid,ASE_bond_hybrids[indx])
                    ...
                    for z,atm in enumerate(ASE_atms):
                        if atm.index == int(ASE_bond_atms[indx][k]) and atm.index not in skip_indices:
                            ASE_new_positions.append([int(TRAP_bond_atms[i][j]),atm.mass,TRAP_bond_charges[i][j],atm.position])
                            skip_indices.append(atm.index)
                            
        ASE_bond_dist_list.pop(indx)
        ASE_bond_atms.pop(indx)
        ASE_bond_hybrids.pop(indx)
